...
char clientID[] = "59b56bd5-07e1-4bba-8583-6b8a9dada9f0";   // Client ID to connect to broker
...
bool connected = false;        // true if connection is active (can be false due to wifi, IP, MQTT problems etc.
int  wifi_failed_attempts=1;   // "0" indicates connected wifi


///// CONSTANTS /////
#define max_wifi_attempts_reboot 5      // After been connected, number of retries before reset/reboot
...
pinMode(LEDG, OUTPUT);        // configure Red Led as output
...
void loop()
{
  ...
  if ( Serial.available() )
  {
	...
	if ( WiFi.status() == WL_CONNECTED && client.connected() ) {           // check if we have connection with the AP
		wifi_failed_attempts=0;                     //Resets the wifi fail counts. "0" indicates connected
		// MQTTconnection();                        // call MQTTconnection function for connection to Mqtt Broker
	...
	else                                                  // when connection with AP lost
    {
	    ...
	    wifi_failed_attempts++;                     //increase the wifi fail count
	    //Reconnect();                                         //Attempt to reconnect - MUST BE IMPLEMENTED
          
	    if (wifi_failed_attempts > max_wifi_attempts_reboot)   //Decide to abort and reboot
                Serial.println("Reseting...");
	        WDTCTL = 0;                                        // zero out the WatchDog Timer so the device restarts
    }    // END - else if ( WiFi.status() == WL_CONNECTED)
...
*/
  ...
  if ( strcmp(topic, HVAC_cmd_topic) == 0 )     // checks if a command type message has been received
  {
     ...
     recvJson = aJson.parse(json);               // create a JSON object from the received message  
     HVAC_cmd();                              //?? (calls the command function without parameters? 
    ...
    aJsonObject* NodeID = aJson.getObjectItem(recvJson, "NodeID"); // read the NodeID value
     ...
     char* MAC=NodeID->valuestring;       // store mac value
      if ( strcmp(MAC, Node_ID) == 0 ) {  // compare mac from message with device mac
         aJsonObject* NodeNo = aJson.getObjectItem(recvJson, "NodeNo");    // Read OnOff value from server
        ...
        digitalWrite(LEDG, HIGH);                                          // turn on Green Led to indicate success to the user
      } else { digitalWrite(LEDR, HIGH); digitalWrite(LEDG, LOW); }        // if check has wrong message count turn on Red Led to inform the user
...
void HVAC_cmd(){
    ...
    aJsonObject* NodeNo = aJson.getObjectItem(recvJson, "NodeNo"); // check the NodeNo
     if (NodeNo != NULL)   {              // if NodeID is in the message 
      if (NodeNo->valueint == Node_No ){  // if NodeNo from message is the same with device NodeNo 
        aJsonObject* msgCnt = aJson.getObjectItem(recvJson, "msgCount"); // store to msgCnt the value received from server  
         if (msgCnt->valueint == (msgCount-1)) {                // Check for lost messages
        Serial.println("\n*OK*\n");                          // print to serial console OK message
        digitalWrite(GREEN_LED, HIGH);                       // turn on Green Led to indicate success to the user
      } else { digitalWrite(RED_LED, HIGH); digitalWrite(GREEN_LED, LOW); } // if check has wrong message count turn on Red Led to inform the user
